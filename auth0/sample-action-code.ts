...
const DB_NAME = "<Auth0 DB connection>"; 
...
exports.onExecuteCustomTokenExchange = async (event, api) => {
  ...
  try {
    const { payload, protectedHeader } = await jwtVerify(subject_token, JWKS, {
      issuer: JWT_ISSUER,
      algorithms: ["ES256"],
    });
    ...
    if (!["kya+JWT", "kya+pay+JWT"].includes(protectedHeader.typ)) {
      ...
      return api.access.deny("invalid_typ", "typ should be one of kya+JWT or kya+pay+JWT");
    }
    ...
    return api.access.deny("invalid_token", "JWT verification failed: invalid token.");
  }
  ...
  if (decodedPayload.env !== "qa") {
    console.log("Invalid environment:", decodedPayload.env);
    return api.access.deny("invalid_env", "Token is not from QA environment.");
  }
  ...
  if (typeof decodedPayload.iat !== "number" || decodedPayload.iat > now) {
    ...
    return api.access.deny("invalid_iat", "Issued-at time is in the future or missing.");
  }
  ...
  if (!validator.isUUID(decodedPayload.jti)) {
    ...
    return api.access.deny("invalid_jti", "Invalid token ID (jti): not a valid UUID.");
  }

  // Validate sub is a UUID
  if (!validator.isUUID(decodedPayload.sub)) {
    console.log("Invalid sub:", decodedPayload.sub);
    return api.access.deny("invalid_sub", "Invalid subject (sub): not a valid UUID.");
  }

  // Validate aud is a UUID
  if (!validator.isUUID(decodedPayload.aud)) {
    console.log("Invalid aud:", decodedPayload.aud);
    return api.access.deny("invalid_aud", "Invalid audience (aud): not a valid UUID.");
  }
  ...
  api.authentication.setUserByConnection(
    DB_NAME, // Auth0 DB connection
    {
      ...
      name: decodedPayload.bid.nameFirst + " " + decodedPayload.bid.nameLast,
